CREATE OR REPLACE PACKAGE PKG_PAR_BOOK IS
  TYPE parlist IS TABLE OF clk_pa_parlist%ROWTYPE;
  current_parlist parlist;
  IptalBayrak boolean;
  d_test  varchar2(1) := 'E';

  procedure muhasebe(p_ekran in varchar2,
                     p_glb_sube in number,
                     p_parlist in parlist);
  procedure muhasebe_fiskes(p_i_sube_kod in number,
                            p_isl_tar in date,
                            p_ref_kod in varchar2,
                            p_isl_ref in number,
                            p_op_adi in varchar2,
                            p_onay_op_adi in varchar2 DEFAULT NULL,
...
END;
/
CREATE OR REPLACE PACKAGE BODY PKG_PAR_BOOK Is
...
--        v_mesaj := GLOBAL.getstdhata;
  ...
  BEGIN
    ...
    ELSIF p_fis_source_type='HESAP' Then
      ...
      BEGIN
      retval := pkg_par_book.dinamiksql(d_source,'DEGER');
      EXCEPTION WHEN OTHERS Then
  ...
  FUNCTION get_parlist(p_ekran   In VARCHAR2,
    ...
    CURSOR c Is
      Select voucher_par_name,voucher_source_type,voucher_source
        From clk_pa_screen_group_book_vouc
  ...
  BEGIN
    ...
    For r In c LOOP
      ...
      Select type_ Into d_ara From clk_pa_screen_fields
    ...
    RETURN d_parlist;
  EXCEPTION WHEN OTHERS Then
    hata_yaz(501,SQLERRM,'PARAMETRE L?STES? OLU?TURULAMADI!',
             p_EKRAN,p_GRUP,Null,p_MUH_ID);
  END;
  ...
  BEGIN
    For r In c LOOP
      ...
      END LOOP;
      BEGIN
      IF pkg_par_book.dinamiksql(d_source,'KOSUL')='0' Then
        RETURN FALSE;
      END IF;
      EXCEPTION WHEN OTHERS Then
  ...
  BEGIN
    ...
    IF kosul_uygun(p_ekran,p_grup,p_muh_id,p_parlist) Then
      ...
      For r In c LOOP
        ...
        EXCEPTION WHEN OTHERS Then
          hata_yaz(401,SQLERRM,'FÝÞ KESMEDE HATA.'||CHR(13)||CHR(10)||r.voucher_par_name||'\'||r.voucher_source_type||CHR(13)||CHR(10)||r.voucher_source||CHR(13)||CHR(10)||
  ...
  BEGIN
    ...
    For r In (Select * From clk_pa_screen_group Where screen=p_ekran Order By order_) LOOP
    BEGIN
...
/*
    CURSOR c Is
      Select * From pm_TEMPORARY Order By kayit_no;
    d_error_no   NUMBER;
    d_valor     DATE;
    d_sayi      NUMBER;
    d_saat      VARCHAR2(8);
    d_op_adi      VARCHAR2(20);
    d_onay_op_adi VARCHAR2(20);
  BEGIN
    Select COUNT(*) Into d_sayi
      From pm_process;
    IF d_sayi=0 Then  --parametrik muhasebede hata yok ise fi? kesilecek...
      IF p_op_adi Is Null Or p_onay_op_adi Is Null Or p_op_adi = p_onay_op_adi Then
        BEGIN
        Select op_kod,son_op_kod
          Into d_op_adi,d_onay_op_adi
          From glb_onay
         Where tarih=p_isl_tar
           And sube_kod=p_i_sube_kod
           And ref_kod=p_ref_kod
           And k_isl_ref=p_isl_ref;
        EXCEPTION WHEN OTHERS Then
          d_op_adi      := p_op_adi;
          d_onay_op_adi := p_onay_op_adi;
        END;
      Else
        d_op_adi      := p_op_adi;
        d_onay_op_adi := p_onay_op_adi;
      END IF;
      BEGIN
          M2_Sbcari.SC_FISI_EKLE(p_i_sube_kod,p_isl_tar);
      EXCEPTION WHEN OTHERS Then
        ROLLBACK;
        M2_Genel.hata_yaz(61175,SUBSTR(SQLERRM,1,45),Null,Null,'E');
      END;
      IF Not(M2_Fis_Kontrol.TMP_Balans_kontrol) Then
        ROLLBACK;
        M2_Genel.hata_yaz(352,Null,Null,Null,'E');
      END IF;
      IF Not(M2_Fis_Kontrol.TMP_Sube_Balans_kontrol) Then
        ROLLBACK;
        M2_Genel.hata_yaz(61194,Null,Null,Null,'E');
      END IF;
      IF M2_Fis_Kontrol.TMP_Alt_ust_kontrol Is Not Null Then
        ROLLBACK;
        M2_Genel.hata_yaz(61195,Null,Null,Null,'E');
      END IF;
      IF Not(M2_Fis_Kontrol.Tarih_kontrol(p_isl_tar, p_i_sube_kod)) Then
        ROLLBACK;
        M2_Genel.hata_yaz(351,Null,Null,Null,'E');
      END IF;
      d_saat := TO_CHAR(SYSDATE,'hh24:mi:ss');
      For r In c LOOP
        IF r.valor Is Not Null Then
          d_valor := r.valor;
        ELSIF r.ek_no > 9000 Or r.isl_kod < 50 Then
          d_valor  := p_isl_tar;
        Else
          d_valor := M2_Tarih.sonraki_isgunu(p_isl_tar);
        END IF;
        IF r.sube_kod Is Null Then
          ROLLBACK;
          M2_Genel.hata_yaz(61191,'SUBE KOD',Null,Null,'E');
        END IF;
        IF r.hesap_no Is Null Then
          ROLLBACK;
          M2_Genel.hata_yaz(61191,'HESAP NO',Null,Null,'E');
        END IF;
        IF r.ek_no Is Null Then
          ROLLBACK;
          M2_Genel.hata_yaz(61191,'EK NO',Null,Null,'E');
        END IF;
        IF r.isl_kod Is Null Then
          ROLLBACK;
          M2_Genel.hata_yaz(61191,'ISLEM KODU',Null,Null,'E');
        END IF;
        IF r.tutar Is Null Then
          ROLLBACK;
          M2_Genel.hata_yaz(61191,'TUTAR',Null,Null,'E');
        END IF;
        IF r.kayit_no Is Null Then
          ROLLBACK;
          M2_Genel.hata_yaz(61191,'KAYIT NO',Null,Null,'E');
        END IF;
        IF r.bacak_grup Is Null Then
          ROLLBACK;
          M2_Genel.hata_yaz(61191,'BACAK GRUP',Null,Null,'E');
        END IF;
        IF p_isl_tar Is Null Then
          ROLLBACK;
          M2_Genel.hata_yaz(61191,'ISLEM TARIHI',Null,Null,'E');
        END IF;
        IF p_ref_kod Is Null Then
          ROLLBACK;
          M2_Genel.hata_yaz(61191,'REF.KOD',Null,Null,'E');
        END IF;
        IF p_isl_ref Is Null Then
          ROLLBACK;
          M2_Genel.hata_yaz(61191,'ISL.REF.',Null,Null,'E');
        END IF;
        IF p_i_sube_kod Is Null Then
          ROLLBACK;
          M2_Genel.hata_yaz(61191,'ISL.SUBE KOD',Null,Null,'E');
        END IF;
        IF d_op_adi Is Null And d_onay_op_adi Is Null Then
          ROLLBACK;
          M2_Genel.hata_yaz(61191,'OPR.KOD',Null,Null,'E');
        END IF;
        IF ROUND(r.tutar,2)<>0 Then
          M2_Fis.fis_kes(r.sube_kod,
                         r.hesap_no,
                         r.ek_no,
                         r.detay_kod,
                         r.isl_kod,
                         ...
                         r.ekkod2,
                         r.symkod,
                         r.bacak_grup, -- bacak gruplar?
                         r.temel_hesap, -- temel hesap
                         r.kayit_no,
                         p_isl_tar,
                         p_i_sube_kod,
                         p_ref_kod,
                         ...
                         SYSDATE,
                         d_saat,
                         NVL(d_op_adi,d_onay_op_adi),
                         NVL(d_onay_op_adi,d_op_adi),
                         d_error_no,
                         Null,
                         p_control);
          IF d_error_no != 0 Then
            ROLLBACK;
            M2_Genel.hata_yaz(d_error_no,TO_CHAR(r.sube_kod)||' '||r.hesap_no||' '||TO_CHAR(r.ek_no)||' '||r.detay_kod,Null,Null,'E');
          END IF;
        END IF;
      END LOOP;
      Pack_Bloke.rezervasyon_kontrol(p_isl_tar,p_ref_kod,p_isl_ref,p_i_sube_kod);
    Else
      ROLLBACK;
      M2_Genel.hata_yaz(61124,Null,Null,Null,'E');
    END IF;
*/ begin null;
  END;
